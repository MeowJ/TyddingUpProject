...
finish_joints_deg = [-169.83,31.2, -104.93, 3.28, -108.78, -114.93]
finish_joints_deg2 = [169.78, 31.2, -104.93, 3.28, -108.78, -114.93]
# Convert joint angles from deg to rad
start_joints_rad = []
for joint in start_joints_deg:
    start_joints_rad.append( round( joint * (math.pi/180),3) )
...
finish_joints_rad2 = []    
for joint in finish_joints_deg2:
    finish_joints_rad2.append( round( joint * (math.pi/180),3) )
...
with eva.lock():
    ...
    if direction==0:
        ...
        eva.control_go_to(finish_joints_rad2, True, 0.2, None)
    eva.control_wait_for_ready()
