...
def main():
    ...
    pub_base = rospy.Publisher('/joy_teleop/cmd_vel', Twist, queue_size=1)
    time.sleep(2)
    ...
    while continue_loop:
        ...
        if len(cnts) > 0:
            ...
            if radius > 10:
                ...
                pixel_world = P2W(x, y)
                if flag_base:
                    # sending message to mobile base ****************************************************************************************
                    if (pixel_world[0] < -15 or  pixel_world[0] > 15 or pixel_world[1] > 15 or pixel_world[1] < -10):
                        twist = Twist()
                        twist.linear.x = float(0 - pixel_world[0]) * base_speed
                        twist.linear.y = float(5 - pixel_world[1]) * base_speed
                        twist.linear.z = 0
                        twist.angular.x = 0
                        twist.angular.y = 0
                        twist.angular.z = 0
                        print('Publishing base_move_to...')
                ...
                if flag_eva:
                    ...
                    if not rospy.is_shutdown():
                        target_coords.data = [robot_target_x, robot_target_y, robot_target_z, robot_target_o,robot_speed]
